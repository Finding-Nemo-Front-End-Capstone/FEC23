...
describe('Questions & Answers', () => {
  ...
  it('should render answers list', async () => {
    const { getByTestId, queryByTestId } = await render(<AnswersList question_id={question.question_id} />);
    jest.mock('axios');
    await waitFor(() => expect(queryByTestId('answers-list-meow')).toHaveTextContent('A:'));
  });
});
